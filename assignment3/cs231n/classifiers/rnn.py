...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        h1, cache_rnn = rnn_forward(word, h0, Wx, Wh, b)
    elif self.cell_type == 'lstm':
        h1, cache_rnn = lstm_forward(word, h0, Wx, Wh, b)
    scores, cache_scores = temporal_affine_forward(h1, W_vocab, b_vocab)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in xrange(max_length):
        word, _ = word_embedding_forward(caption_in, W_embed)
        if self.cell_type == 'rnn':
