...
def batchnorm_backward_alt(dout, cache):
  ...
  #############################################################################
  if mode == 'train':
      _, x, gamma, xc, std, xhat, out = cache
      dbeta = dout.sum(axis=0)
      dgamma = np.sum(xhat * dout, axis=0) 
      N = x.shape[0]
      dx = 1./N * gamma / std * (N * dout - np.sum(dout, axis=0) - xc / (std ** 2) * np.sum(dout*xc, axis=0))
  elif mode == 'test':
      _, _, xhat, gamma, beta, std = cache
      dbeta = dout.sum(axis=0)
      dgamma = np.sum(xhat * dout, axis=0)
      dx = gamma * dout / std
  else:
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in xrange(N):
     for k in xrange(F):
          for j in xrange(C):
              x_with_pad = np.pad(x[i,j,:], pad_width=pad, mode='constant', constant_values=0)
              for hout in xrange(H_out):
                  for wout in xrange(W_out):
                      roff = stride*hout
                      coff = stride*wout
                      out[i,k,hout,wout] += np.sum(x_with_pad[roff:HH+roff,coff:WW+coff] * w[k,j,:])
          out[i,k,:] += b[k]
          ...
          for k in xrange(F):
              for hout in xrange(H_out):
                  for wout in xrange(W_out):
                      roff = stride*hout
                      coff = stride*wout
                      dw[k,j,:] += dout[i,k,hout,wout]*x_with_pad[roff:HH+roff,coff:WW+coff]
              ...
              for f in xrange(F):
                  for hout in xrange(H_out):
                      for wout in xrange(W_out):
                          ...
                          dx[i, :, j, k] += dout[i, f, hout, wout] * w_masked 
...
def max_pool_forward_naive(x, pool_param):
  ...
  N,C,H,W = x.shape
  H_out = (H-pheight)/stride + 1
  W_out = (W-pwidth)/stride + 1
      ...
      for j in xrange(C):
          for hout in xrange(H_out):
              for wout in xrange(W_out):
                  roff = stride*hout
                  coff = stride*wout
                  out[i,j,hout,wout] = np.max(x[i,j,:][roff:pheight+roff, coff:pwidth+coff])
      ...
      for j in xrange(C):
          for hout in xrange(H_out):
              for wout in xrange(W_out):
                  roff = stride*hout
                  coff = stride*wout
                  x_sub = x[i,j,roff:pheight+roff, coff:pwidth+coff]
                  idx = np.argmax(x_sub)
                  h,w = np.unravel_index(idx, x_sub.shape)
                  dx[i,j,roff+h,coff+w] = dout[i,j,hout,wout]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
