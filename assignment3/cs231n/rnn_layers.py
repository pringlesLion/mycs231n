...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  x = x.transpose(1,0,2)
  h = np.zeros((T,N,H))
  h[0], cache_0 = rnn_step_forward(x[0,:], h0, Wx, Wh, b)
  prev_h = h[0]
  cache = []
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = (x, prev_h, prev_c, Wx, Wh, a, ai, af, ao, ag, i, f, o, g, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  """
  x, prev_h, prev_c, Wx, Wh, a, ai, af, ao, ag, i, f, o, g, next_c, next_h = cache  
  #############################################################################
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
      dh_current = dh_prev + dh[t]
      dc_current = dc_prev
      dx[t], dh_prev, dc_prev, dWx_t, dWh_t, db_t = lstm_step_backward(dh_current, dc_current, cache[t])
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
